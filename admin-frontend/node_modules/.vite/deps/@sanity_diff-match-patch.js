...
function bisectSplit(text1, text2, x, y, deadline) {
  const text1a = text1.substring(0, x), text2a = text2.substring(0, y), text1b = text1.substring(x), text2b = text2.substring(y), diffs = doDiff(text1a, text2a, { checkLines: false, deadline }), diffsb = doDiff(text1b, text2b, { checkLines: false, deadline });
    ...
    for (; bigpatch.diffs.length !== 0; ) {
      ...
      for (; bigpatch.diffs.length !== 0 && patch.length1 < patchSize - margin; ) {
        ...
        if (diffType === DIFF_INSERT) {
          ...
          diff2 && patch.diffs.push(diff2), empty = false;
...
function apply(patches, originalText, opts = {}) {
  ...
  for (let x = 0; x < parsed.length; x++) {
    ...
    if (text1.length > MAX_BITS ? (startLoc = match(text, text1.substring(0, MAX_BITS), expectedLoc), startLoc !== -1 && (endLoc = match(
      ...
      results[x] = false, delta -= parsed[x].length2 - parsed[x].length1;
    else {
      results[x] = true, delta = startLoc - expectedLoc;
      let text2;
...
//# sourceMappingURL=@sanity_diff-match-patch.js.map
