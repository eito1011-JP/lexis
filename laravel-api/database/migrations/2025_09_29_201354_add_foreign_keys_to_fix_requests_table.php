...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('fix_requests', function (Blueprint $table) {
            $table->foreign(['base_category_version_id'])->references(['id'])->on('document_categories')->onUpdate('no action')->onDelete('set null');
            $table->foreign(['base_document_version_id'])->references(['id'])->on('document_versions')->onUpdate('no action')->onDelete('set null');
            $table->foreign(['document_category_id'])->references(['id'])->on('document_categories')->onUpdate('no action')->onDelete('set null');
